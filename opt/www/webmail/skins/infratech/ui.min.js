...
;(function(){var t=setInterval(function(){if(typeof tinymce==="undefined")return;var ed=tinymce.get("composebody");if(ed===null||ed===undefined)return;if(ed._bqe_bound)return;ed._bqe_bound=true;clearInterval(t);ed.on("keydown",function(ev){var node=ed.selection.getNode();var el=node;while(el&&el!==ed.getBody()){if(el.nodeName==="BLOCKQUOTE"){ev.preventDefault();ev.stopImmediatePropagation();return false;}el=el.parentNode;}},true);ed.on("paste",function(ev){var node=ed.selection.getNode();var el=node;while(el&&el!==ed.getBody()){if(el.nodeName==="BLOCKQUOTE"){ev.preventDefault();return false;}el=el.parentNode;}},true);},300);})();
